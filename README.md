...
这是 ALSA 的行为，不是合成的问题。播完关流的时候，ALSA 会把内核缓冲区里
还没播出去的数据直接扔掉——`portaudio` 的 `stop()` 只保证它自己那一层队列
排空了，管不到 ALSA 那边。所以听感就是每句话尾巴上少一截，句子越长越明显。

`PLAYBACK_DRAIN`（默认 0.3 秒）就是治这个的：关流前往尾巴上补一段静音当
替死鬼，再等声卡缓冲区那么久，等真正的内容已经出了设备才关流。丢的只是
那段静音。这个值调大更保险，代价是每句话结尾多一段静音。

在树莓派上用 `bench_play.py` 验证：它放的是 1.5 秒低音加三声"嘀"，
三声都听得见就没问题，少了几声就把 `PLAYBACK_DRAIN` 调大，
`--drain 0.8` 这样再听一遍对比。`samples/play_tail.wav` 是同一段声音，
可以拷到电脑上听原本该是什么样。
